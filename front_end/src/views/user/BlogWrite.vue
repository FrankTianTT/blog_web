...
export default {
  ...
  mounted() {
    // console.log("blogwirte mounted");
    if (this.articleNeedModify) {
      this.articleForm.title = this.articleFormNeedModify.title;
      this.content = this.articleFormNeedModify.content;
      this.articleForm.categoryId = this.articleFormNeedModify.categoryId;
      this.articleForm.labels = this.articleFormNeedModify.label.split(" ");
      console.log(this.articleForm);
    }
  },
  beforeRouteLeave(to, from, next) {
    this.set_articleNeedModify(false);
    console.log("call before route leave");
    next();
  },
  computed: {
    ...mapGetters([
  ...
  methods: {
    ...mapMutations(["set_articleNeedModify"]),
    ...mapActions(["addArticle", "modifyArticle"]),
    change(value, render) {
      //实时获取转成html的数据
      this.html = render;
    ...
    handleConfirm() {
      ...
      if (this.articleNeedModify) {
        data = {
          ...data,
          id: this.articleFormNeedModify.id
        };
        console.log(data);
        this.set_articleNeedModify(false);
      } else {
